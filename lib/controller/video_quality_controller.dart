...
class VideoQualityController extends GetxController {
  ...
  Future<MyAccountResponse> getAccount() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        groupValue.value = result.users?.videoQuality == "SD" ? 1 : 2;
        return result;
      } else {
  ...
  Future<ServerResponseModel> setVideoQuality(String id) async {
    showLoader.value = true;
    var response = await http.post(
        ...
        body: jsonEncode(<String, String>{"videoQuality": id}));
    showLoader.value = false;
    if (response.statusCode == 200) {
